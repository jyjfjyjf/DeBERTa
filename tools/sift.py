...
class PerturbationLayer(paddle.nn.Layer):
    ...
    def update_delta(self, requires_grad=False):
        if not self.adversarial_mode:
            return True
        if self.delta is None:
            delta_input = paddle.normal(0, self.init_perturbation, self.input.shape)
            delta = paddle.clip(delta_input,
            ...
            delta = self.delta
            norm = grad.norm()
            if paddle.isnan(norm) or paddle.isinf(norm):
                return False
            eps = self.learning_rate
            with paddle.no_grad():
                delta = delta + eps * grad / (1e-6 + grad.abs().max(-1, keepdim=True)[0])
        delta = delta.astype('float32').detach()
        delta.stop_gradient = not requires_grad
        self.delta = delta
        self.perturbated_input = (self.input.astype(delta.dtype).detach() + self.delta).astype(self.input.dtype)
        return True
...
def hook_sift_layer(model, hidden_size, learning_rate=1e-4, init_perturbation=1e-2,
  ...
  """
    ...
    if isinstance(target_module, str):
        ...
        _modules = [k for n, k in model.named_sublayers() if target_module in n]
    else:
        assert isinstance(target_module,
                          paddle.nn.Layer), f'{type(target_module)} is not an instance of torch.nn.Module'
...
class AdversarialLearner:
  ...
  """
    ...
    def prepare(self):
        self.prev = [not p.stop_gradient for p in self.parameters]
...
def symmetric_kl(logits, target):
    logit_stu = logits.reshape((-1, logits.shape[-1])).astype('float32')
    logit_tea = target.reshape((-1, target.shape[-1])).astype('float32')
