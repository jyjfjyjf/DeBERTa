...
class BertAttention(nn.Layer):
    ...
    def forward(self,
        ...
        self_output, att_matrix, att_logits_ = output['hidden_states'], output['attention_probs'], output[
            'attention_logits']
...
class ConvLayer(nn.Layer):
    def __init__(self, config):
        ...
        self.conv = nn.Conv1D(config.hidden_size, config.hidden_size, kernel_size, padding=(kernel_size - 1) // 2,
                              groups=groups)
        self.LayerNorm = LayerNorm(config.hidden_size, config.layer_norm_eps)
        self.dropout = StableDropout(config.hidden_dropout_prob)
        # self.dropout = nn.Dropout(config.hidden_dropout_prob)
    ...
    def forward(self, hidden_states, residual_states, input_mask):
        out = transpose(self.conv(transpose(hidden_states, (0, 2, 1))), (0, 2, 1))
        rmask = (1 - input_mask).astype('bool')
        rmask = rmask.unsqueeze(-1).expand(out.shape)

        # 使用paddle实现torch的masked_fill_
        tmp_mask = paddle.full(out.shape, 0, out.dtype)
        out = paddle.where(rmask, tmp_mask, out)
...
class BertEncoder(nn.Layer):
    ...
    def get_attention_mask(self, attention_mask):
        if attention_mask.dim() <= 2:
            ...
            attention_mask = attention_mask.astype('byte')
    ...
    def get_rel_pos(self, hidden_states, query_states=None, relative_pos=None):
        if self.relative_attention and relative_pos is None:
            ...
            relative_pos = build_relative_position(q, hidden_states.shape[-2], bucket_size=self.position_buckets,
    ...
    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True, return_att=False,
        ...
        if attention_mask.dim() <= 2:
            ...
            input_mask = (attention_mask.sum(-2) > 0).astype('byte')
...
class BertLMPredictionHead(nn.Layer):
    def __init__(self, config, vocab_size):
        ...
        self.bias = paddle.fluid.Layer.parameters(paddle.zeros(vocab_size))
    ...
    def forward(self, hidden_states, embeding_weight):
        ...
        # b x s x v
        logits = paddle.matmul(hidden_states, embeding_weight.t().to(hidden_states)) + self.bias
        return logits
